...
def update_param(feature, current_param_dict):
	...
	for m in xrange(0, num_sents):
		...
		for candidate, interval_start, interval_end in sequence:
			interval_dict[(interval_start, interval_end)] = list(bleu.bleu_stats(candidate[2].split(), ref))
		iter_dict[m] = interval_dict

	all_interval_ends = sorted(set([item[1] for sublist in [interval_dict.keys() for interval_dict in iter_dict.values()] for item in sublist]))
	current_best = 0
	best_interval = (0,0)
	for i in range(len(all_interval_ends)):
		all_stats = collect_BLEU_stats(iter_dict, all_interval_ends[i])
		new_BLEU = 100*bleu.bleu(all_stats)
		if new_BLEU > current_best:
			current_best = new_BLEU
			best_interval = (all_interval_ends[i-1], all_interval_ends[i])
	current_param_dict[feature] = sum(best_interval)/2
	return current_param_dict, current_best
